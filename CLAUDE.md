...
4. ~~Re-baseline `cmd/benchclient` against the real NLB address~~ — **Done (2026-07-06).** First run from an external client (laptop, over the public internet, `-concurrency 16 -requests 2000`) showed a severe apparent regression: p50=42.2ms, p90=77.0ms, **p99=5.0s**, 112/2000 errors (5.6%). Dropping to `-concurrency 4 -requests 200` eliminated all errors but p50/p90/p99 stayed at 39.5ms/41.8ms/235.3ms — ruling out server-side overload/throttling as the cause (throughput was flat between 4 and 16 concurrency, and zero errors at low concurrency rules out capacity exhaustion). **Root-caused by running `benchclient` from inside the cluster instead** (`kubectl run` a throwaway pod, hitting the Service's internal DNS name `spatialscale-server:50051` rather than the external NLB hostname): p50=0.40ms, p90=1.02ms, **p99=13.4ms** — matching (p50 even beating) the Phase A localhost numbers. **Conclusion: the service and cluster-internal networking are exactly as fast as Phase A predicted; the 39ms+ external number was overwhelmingly the client's real internet round-trip latency to `us-east-1` (Colorado-to-Virginia physical distance), not an architecture or infra problem.** This is the honest, correct framing for the resume claim: "sub-10ms p99" holds for the service/cluster, not for "arbitrary home-internet client to us-east-1" — same caveat structure as the Phase A box-size finding (the claim holds for the realistic interactive use case, not for a different, harder scenario). No further latency optimization needed; skipped step 5's optimization list entirely since there was no real regression to fix.
